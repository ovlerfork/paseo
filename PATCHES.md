...
- When `Upstream Sync` detects a new upstream commit, it passes that full SHA and an explicit development-publish input to `Patch Check`. After patch application, sanitization, and shell validation succeed, Patch Check dispatches development publishing directly. Development source image tags are `:dev` and `:dev-<full-upstream-sha>` for the default variant, plus `:dev-ubuntu-sandbox` and `:dev-<full-upstream-sha>-ubuntu-sandbox` for the Ubuntu sandbox variant. The workflow skips the source-image build only when both immutable development tags already exist.
